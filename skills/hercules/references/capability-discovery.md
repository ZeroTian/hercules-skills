...
This is an unskippable completion gate: Hermes must not select a route until the facility has explicit current broad-role evidence and authority and every concrete requirement has task-relevant local surface evidence. Missing authority or evidence is not `read-only` or `local-metadata`; it is ineligible. Executable, CLI presence, or version-only evidence is container evidence and cannot prove a specialized surface. If every plausible relevant surface family has a confirmed absence, reject the candidate. Record absence without inspecting credentials, secret values, login state, or provider reachability. User and project preferences rank only candidates that already pass this gate.
...
Record broad role, `required_capabilities`, facility, concrete confirmed surfaces, authority class, evidence source, missing requirements, and freshness for this session only. Reuse a cache record only when its requirement set exactly matches the current demand and its current supporting surfaces cover every requirement. Filter unrelated cached surfaces from the returned map.
