...
| portable-skill-pack-installation | atom (core) | One-command installer, setup/doctor UX, dependency alignment, dry-run safety, runtime symlink/copy install, auth handoff, validation/review gates. |
| cli-installer-ux-governance | atom (core) | Productized CLI installer UX: setup/doctor/status command shape, read-only diagnosis, repair boundaries, auth handoff, JSON/strict automation output, and independent review checks. |
